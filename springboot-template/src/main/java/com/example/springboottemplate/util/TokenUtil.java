...
public class TokenUtil {
  ...
  // Token过期时间
  private static final long EXPIRE_TIME = 24 * 60 * 60 * 10 * 1000;
  ...
  public static String sign(String username, int id) {
    try {
      ...
      // 返回token字符串
      return JWT.create().withHeader(header).withClaim("username", username).withClaim("id", id)
          // 过期时间
          .withExpiresAt(new Date(System.currentTimeMillis() + EXPIRE_TIME)).sign(Algorithm.HMAC256(TOKEN_SECRET));
    } catch (Exception e) {
      e.printStackTrace();
      return null;
    }
   ...
   * 检验是否更新token
   */
  public static boolean verify(String token, String username, int id) throws JWTVerificationException {
    Algorithm algorithm = Algorithm.HMAC256(TOKEN_SECRET);
    JWTVerifier verifier = JWT.require(algorithm).withClaim("username", username).withClaim("id", id).build();
