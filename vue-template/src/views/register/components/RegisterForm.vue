<template>
  <el-form ref="form" :model="form" :rules="rules" label-position="left">
    ...
    <el-button
      ...
      type="primary"
      @click="register"
    >{{ loading ? '注 册 中...' : '注 册' }}</el-button>

    <div class="flex" style="margin-top: 20px">
      <p />
      <el-button style="float:right" type="text" @click="login">已有账户登陆</el-button>
    </div>
  </el-form>
</template>
...
import { checkName, register } from "@/api/user";
  ...
  methods: {
    register() {
      ...
      this.$refs.form.validate(valid => {
        if (!valid) return;
        this.loading = true;
        register(this.form.username, this.form.pwd)
          .then(() => {
